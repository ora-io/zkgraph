...
function calcPrice(syncEvent: Event): BigInt{
    ...
    let price0 = r0.times(token0_factor).times(price_factor).div(r1.times(token1_factor))
...
export function handleEvents(events: Event[]): Bytes {
  ...
  if (lastSyncEvent == null) { // Don't Trigger if there's no event in the block
    require(false)
    ...
    let triggerCondition = price0.ge(BigInt.fromI32(threshold_eth_price * 10**price_decimals))
