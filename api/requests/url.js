...
export default {
  postNewWasmImage: () => new EndPoint(`${config.ZkwasmProviderUrl}/setup`, false),
  fetchConfiguredMD5: () =>
    new EndPoint(`${config.ZkwasmProviderUrl}/tasks?tasktype=Setup`, false),
  checkWasmImageStatus: (md5, taskType = null) =>
    new EndPoint(
      `${config.ZkwasmProviderUrl}/tasks?md5=${md5}${taskType ? "&tasktype=" + taskType : ""}`,
      false
    ),
  deployWasmImageURL: () =>
  ...
  getUserBalance: (address) =>
    new EndPoint(`${config.ZkwasmProviderUrl}/user?user_address=${address}`, false),
  sendTXHash: (address) =>
